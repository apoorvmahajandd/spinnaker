...
public class ArtifactProvider {
   ...
   * kubectl get -l works better than doing a kubectl get and then filtering the result. The
   * downside is that if the provided labels that are supplied in this map change for any reason,
   * then this kind of filtering could return incomplete values since the label selector may not
   * match the labels on the previously created resources. So care must be taken when using this
   * option. Ideally, it would be great if we could either select on annotations, or kubectl
   * supports a better way to find manifests with regexes, but until then this is the workaround.
   *
